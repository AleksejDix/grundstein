...
test("user sees total payments for the year", async () => {
  ...
  await expect
    .element(screen.getByText(/17\.301,12/))
    .toBeInTheDocument()

  await expect
    .element(screen.getByText("12 Monate"))
    .toBeInTheDocument()
})
...
test("user sees total interest paid in first year", async () => {
  ...
  await expect
    .element(screen.getByText("Zinsen gesamt"))
    .toBeInTheDocument()

  // User should see interest amount
  await expect
    .element(screen.getByText(/5\.799,96/))
    .toBeInTheDocument()
...
test("user sees total principal paid in first year", async () => {
  ...
  await expect
    .element(screen.getByText("Tilgung gesamt"))
    .toBeInTheDocument()
...
test("user sees remaining balance after first year", async () => {
  ...
  await expect
    .element(screen.getByText("Restschuld"))
    .toBeInTheDocument()

  // User should see remaining balance
  await expect
    .element(screen.getByText(/288\.498,84/))
    .toBeInTheDocument()

  await expect
    .element(screen.getByText("Nach Jahr 1"))
    .toBeInTheDocument()
})
...
test("user sees progress summary showing loan paydown", async () => {
  ...
  await expect
    .element(screen.getByText(/getilgt/))
    .toBeInTheDocument()
})
