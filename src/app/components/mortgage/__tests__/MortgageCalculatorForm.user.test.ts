...
test('shows mortgage calculator heading', async () => {
  ...
  await expect.element(screen.getByText('Mortgage Calculator')).toBeInTheDocument()
})
...
test('shows calculate button', async () => {
  ...
  await expect.element(screen.getByRole('button', { name: /calculate monthly payment/i })).toBeInTheDocument()
})
...
test('user can fill in complete form', async () => {
  ...
  const interestRateInput = screen.getByLabelText(/annual interest rate.*zinssatz/i)
