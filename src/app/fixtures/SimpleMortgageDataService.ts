...
import { getAvailablePercentages } from "../../core/domain/types/ExtraPaymentRules";
...
export type SimpleMortgageData = {
  ...
  extraPaymentLimit: ExtraPaymentLimit;
};
...
export function createSimpleApartmentMortgage(): SimpleMortgageData {
  ...
  if (!loanAmountResult.success || !interestRateResult.success || 
      !principalResult.success || !interestResult.success || !totalResult.success) {
  ...
  if (!totalPaidResult.success || !currentBalanceResult.success || !equityBuiltResult.success) {
  ...
  const extraPaymentLimit = createPercentageLimit(20);
 ...
 */
export function getExtraPaymentAnalysis(
  ...
  const availablePercentages = getAvailablePercentages();
  
  // Simple recommendation: use 10% if available
  const recommendedPercentage = mortgage.extraPaymentLimit.type === "Percentage" ? 
    Math.min(10, mortgage.extraPaymentLimit.value) : 10;
