...
if (extraPayment.success) {
  ...
  console.log(
    `Extra €${toEuros(payment.amount)} in month ${toNumber(payment.paymentMonth)}`,
  );
}
```
...
if (period.success) {
  ...
  console.log(
    `Fixed ${formatInterestRate(fix.fixedRate)} for months ${toNumber(fix.startMonth)}-${toNumber(fix.endMonth)}`,
  );
}
```
...
if (valuation.success) {
  ...
  const maxLoan = calculateMaxLoanAmount(
    prop.marketValue,
    prop.loanToValueRatio,
  );
  console.log(
    `Max loan: ${toEuros(maxLoan)} at ${toPercentage(prop.loanToValueRatio)} LTV`,
  );
}
```
...
if (amount.success && rate.success && term.success) {
  ...
  if (config.success) {
    console.log(
      `Calculated payment: ${toEuros(config.data.monthlyPayment.totalAmount)}`,
    );
  }
}
```
...
if (valuation.success) {
  ...
  console.log(
    `Max loan at ${toPercentage(valuation.data.loanToValueRatio)} LTV: ${toEuros(maxLoan)}`,
  );
}
```
...
fc.assert(
  fc.property(validLoanConfigArbitrary, (config) => {
