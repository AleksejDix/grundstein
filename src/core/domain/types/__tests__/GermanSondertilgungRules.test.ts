...
describe("GermanSondertilgungRules", () => {
  describe("createGermanSondertilgungRules", () => {
    ...
    it("should create rules for Privatbank with fixed fees", () => {
      ...
      if (result.success) {
        ...
        const baseFeeResult = createMoney(250);
        expect(baseFeeResult.success).toBe(true);
        if (baseFeeResult.success) {
          expect(rules.feeStructure.baseFee).toEqual(baseFeeResult.data);
        }
      }
    });
    ...
    it("should allow custom overrides", () => {
      const minimumAmountResult = createMoney(5000);
      expect(minimumAmountResult.success).toBe(true);
      if (!minimumAmountResult.success) return;
      
      const customRules = {
        allowedPercentages: [15, 25] as const,
        minimumAmount: minimumAmountResult.data,
      };
  ...
  describe("validateSondertilgungPayment", () => {
    const createTestLoanAmount = () => {
      const result = createLoanAmount(300000);
      if (!result.success) throw new Error("Failed to create test loan amount");
      return result.data;
    };
    const createTestRules = () => {
      const result = createGermanSondertilgungRules("OnlineBank");
      if (!result.success) throw new Error("Failed to create test rules");
      return result.data;
    };
