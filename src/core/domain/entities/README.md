...
**Last Updated:** 2025-07-12 - Entity documentation updated to reflect current patterns.
...
const updated = updateMortgageInPortfolio(portfolio, "mortgage_456", {
...
type PortfolioSummary = {
  ...
  readonly maturityRange: {
    // Loan maturity spread
    earliest: Date;
    latest: Date;
  };
};
...
let portfolio = createMortgagePortfolio(
  "multi_market",
...
if (optimization.success) {
  ...
  opportunities.refinancingCandidates.forEach((candidate) => {
    console.log(
      `Mortgage ${candidate.mortgageId}: Save ${toEuros(candidate.potentialSavings)}`,
    );
  });
  ...
  if (opportunities.consolidationBenefit.worthwhile) {
    console.log(
      `Consolidation could save: ${toEuros(opportunities.consolidationBenefit.totalSavings)}`,
    );
  }
}
```
