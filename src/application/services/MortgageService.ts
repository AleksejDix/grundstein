...
import type { LoanConfiguration } from "../../domain/types/LoanConfiguration";
import { createLoanConfiguration } from "../../domain/types/LoanConfiguration";
import type { ExtraPayment } from "../../domain/types/ExtraPayment";
import { createExtraPayment } from "../../domain/types/ExtraPayment";
import type { SondertilgungPlan } from "../../domain/types/SondertilgungPlan";
import type { Money } from "../../domain/types/Money";
import { createMoney, toEuros } from "../../domain/types/Money";
import type { LoanAmount } from "../../domain/types/LoanAmount";
import { createLoanAmount } from "../../domain/types/LoanAmount";
import type { InterestRate } from "../../domain/types/InterestRate";
import { createInterestRate } from "../../domain/types/InterestRate";
import type { MonthCount } from "../../domain/types/MonthCount";
import { createMonthCount } from "../../domain/types/MonthCount";
import type { YearCount } from "../../domain/types/YearCount";
import { createYearCount } from "../../domain/types/YearCount";
import type { PaymentMonth } from "../../domain/types/PaymentMonth";
...
} from "../../domain/calculations/LoanCalculations";
import {
...
export class MortgageService {
    ...
    try {
      ...
      const analysis: LoanAnalysis = {
        ...
        totals: {
          ...
          termInYears:
            Math.round(
              ((input.termMonths || input.termYears! * 12) / 12) * 10
            ) / 10,
        },
        firstYearSummary: {
  ...
  async analyzeSondertilgung(
    ...
    try {
      // Analyze base loan
      const baseLoanResult = await this.analyzeLoan(baseLoan);
  ...
  async compareScenarios(
    ...
    try {
      ...
      for (const scenario of scenarios) {
        const result = await this.analyzeLoan(scenario);
        if (!result.success) {
      ...
      if (paymentRange > 100) {
        insights.push(
          `Monthly payments vary by €${paymentRange.toFixed(
            0
          )} across scenarios`
        );
      }
      ...
      if (interestRange > 1000) {
        insights.push(
          `Total interest varies by €${interestRange.toFixed(
            0
          )} across scenarios`
        );
      }
