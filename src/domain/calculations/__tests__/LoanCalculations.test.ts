...
import { createYearCount } from "../../types/YearCount";
...
describe("LoanCalculations", () => {
  ...
  function createTestLoanConfig(
    ...
    const payment = createMoney(1000); // Placeholder payment - actual calculations will override
  ...
  const zeroInterestLoan = (() => {
    ...
    const rate = createInterestRate(0.0); // 0.0%
    const term = createYearCount(5); // 5 years
    const payment = createMoney(833.33); // €50k / 60 months for 0% interest
